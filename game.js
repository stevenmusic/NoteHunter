...
const YOKAI_DATABASE = [
  { name: "糰子暮泥", icon: "💧", bg: "radial-gradient(circle at 35% 35%, #9ae5f3, #4eaec5)" }, // 史萊姆
...
function executeSpawnNextMonster() {
  ...
  monster.currentHp += monster.maxHp; // 遞補下一隻怪的血量
...
function spawnNextSlime() {
  damageMonster(monster.currentHp); // 相容舊版呼叫
}
...
function simulateOfflineProgress(seconds) {
  ...
  let initialLevel = monster.level;
...
function updateCharacter() {
  ...
  if(statsEl) {
    statsEl.innerHTML = `
      <div style="background:#f5eedf; padding:10px; border-radius:4px; border:2px solid #d4af37; font-weight:bold; color:#1c1b1a; text-align:center; box-shadow: 0 4px 6px rgba(0,0,0,0.3); margin-bottom:12px; font-size:13px; position:relative;">
...
function updateExpUI(){
  ...
  if(expText) expText.innerText = `魂魄能量 ${player.exp} / ${currentLevelMaxExp}`;
}
